...
const {
  storageChainconfig, storageccscconfig, getchainconfig,  getCCSCdatawithHash, getallinfo,storeOC,GetOC
}=require('../service/store.js');
const {GetResFromContract,SetResToOracleContract} = require("../Oracle");
const {getOraclesubCCSC} = require("../service/store");
...
//保存预言合约信息
router.post('/SetOC', async (ctx, next) => {
  ...
  let ress="";
  await storeOC(requestjson).then((res)=>{
    ctx.body={
      msg:"保存成功"
    }
  },(err)=>{
...
router.post('/registerconstract', async (ctx, next) => {
  ...
  await p.then((res)=>{
    ...
    const data={
      ...
      Publickey:"",
      ABI:ctx.request.body.abi,
      ENFlag: ctx.request.body.ENFlag||"0",
      Topic:ctx.request.body.Topic||"0x",
      EventName:ctx.request.body.EventName||"error"
    }
    ...
    throw error})
      ...
      .then((res)=>{
        ...
        if (!res.checksign){
          ...
          console.log(ctx.request.body.abi)
          //  注册成功，记录注册数据
          storageccscconfig({
...
//  首先校验数据是否正确，校验签名
  ...
  if (res.code===0){
    ...
    console.log("接收数据为：",parse.Result)
    ...
    await SetResToOracleContract(parse,parse.input.Flag).then(console.log);
    //整理数据
    reponsedata = {
...
router.post('/multiquery',async (ctx,next)=>{
  ...
  var p = await getCCSCdatawithHash(res.input.CCSCHash);

  for (let i = 0; i < chaindata.length; i++) {
    // console.log(res)
    chaindata[i].result= await GetResFromContract(p,"1",chaindata[i].preargs)
    // console.log(await GetResFromContract(p,"1",item.preargs))
  }
