...
const {mongoose,Chain,CCSC,Invocation,OC}=require('./mongoop.js')
...
* 时间存储
* */
const storagepassivetime =  (data) => {
    return new Promise(async (resolve, reject) => {
        let instance=new Invocation()
        instance.set(data)
        await instance.save((error)=>{
            if (error!=null){
                console.log(error)
...
const getOraclesubCCSC = () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        var time = new Date().getTime();
        //读取CCSC的详细数据
        CCSC.find({ENFlag:"0"},{data:0}).then((docs)=>{
            // console.log(docs)
            if (docs==undefined){
                reject("合约接口数据不存在，请检查相关数据或配置")
            }else {
...
* 保存预言合约信息
* */
const storeOC = (OCinfo) => {
    return new Promise(async (resolve, reject) => {
        let instance=new OC()
        OC.find({},async (err,docs)=>{
            if (docs.length>0){
                instance=docs[0]
            }
            instance.set(OCinfo)
...
module.exports = {
    ...
    getactiveCCSC,storagepassivetime,getOraclesubCCSC,storeOC,GetOC
}
