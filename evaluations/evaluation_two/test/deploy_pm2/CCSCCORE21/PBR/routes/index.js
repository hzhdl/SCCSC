...
const {pbrconfig,pbrconfig1}=require('../config/pbrconfig.js')
...
router.all('/test', async (ctx, next) => {
    ...
    if (res.EFlag=="1") {
        ...
        console.log("----")
        await test1(res.nums, res.flag, res.chainhash, res.Nlimit, res.Climit).then((res) => {
            ctx.body = {
                msg: "success",
                data: res,
            }
...
async function test(i,flag,chainhash,Nlimit,climit) {
    ...
    while(i--){
        ...
        for (let j = 0; j < addrs.length; j++) {
            ...
            ABI=pbrconfig[f].ABI
            promise.push(getresult(url,add,ABI,func,""+f+""))
        }
...
async function test1(i,flag,chainhash,Nlimit,climit) {
    // console.log(i,flag,chainhash,Nlimit,climit)
    let url=''
    let add=''
    let ABI=[]
    ...
    let addrs=[]
    let f=0;
    await requestcontract("8080","/evaluation/getcontractaddre2",{
        "chainhash":chainhash,
        "flag":false,
        "NumsLimit": Nlimit,
        "ChainLimit":climit
    ...
    while(i--){
        ...
        let begintime = new Date().getTime();
        for (let j = 0; j < addrs.length; j++) {
            url=pbrconfig1[f].host+':'+pbrconfig1[f].port
            add=addrs[f]
            func=pbrconfig1[f].func
            ABI=pbrconfig1[f].ABI
            promise.push(getresult(url,add,ABI,func,""+f+""))
        }
        await Promise.all(promise)
            .then((res)=>{
