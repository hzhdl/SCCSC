...
describe("proxy", function () {
    ...
    describe("Deployment contract   ", function () {
        ...
        //     for (let i = 0; i < 50; i++) {
        //         await registerchain("300"+i)
        //     }
        // })
        //  实验二中，为方便验证，禁用数据同步的上链操作，单纯对比几个方案的性能。
        //因此这里不再注册预言合约
        ...
        it("注册订阅合约",async function () {
            ...
            let count=100
            let sublist=[]
            // //实验二订阅   在链一上100个合约，均被30条链订阅
            for (let i = 1; i <= 30; i++) {
                sublist.push("300"+i)
            }
            for (let i = 0; i < count; i++) {
                //eth->eth 这里订阅一个
                await ethtestcontractdeploy("http://172.16.0.13:8640","3000",sublist)
                // await ethtestcontractdeploy("http://172.16.0.13:8645","3001",["3003","3004"])
                sleep(1000);
            }

            //实验二订阅   在链二上，按照10,20,30,40,50个链订阅，分别注册60个合约
            let counts=[
                10,20,30,40,
                50]
            for (const count1 of counts) {
                let subl=[]
                ...
                for (let i = 0; i < 60; i++) {
                    //eth->eth 这里订阅一个
                    // await ethtestcontractdeploy("http://172.16.0.13:8640","3000",sublist)
                    await ethtestcontractdeploy("http://172.16.0.13:8645","3001",subl)
                    sleep(1000);
                }
            }
        })
