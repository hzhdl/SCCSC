...
async function evealresult(chainhash,nl,cl,counts,flag,dsigner) {
    ...
    return await sleep(30000).then(async (res)=>{
        return await requesttimedata("8080","/evaluation/getdatares",{
            "date":begintimes
        })
    })
}
async function evealresult1(chainhash,nl,cl,counts,flag,dsigner) {
    let allcontracts=[]
...
describe("proxy", function () {
    ...
    //         finalresult[i]=[]
    //         for (let j = 0; j < 1; j++) {
    //             await evealresult(Chainhash[0],nl[i],cl,counts,"1",dsigner).then((res)=>{
    //                 finalresult[i].push(res.data.data);
    //             })
    //         }
    //     }
    ...
    it("测试从eth->eth", async function () {
        ...
        for (let i = 0; i < cl.length; i++) {
            finalresult[i]=[]
            for (let j = 0; j < 3; j++) {
                await evealresult1(Chainhash[1],nl,cl[i],counts,"0",dsigner).then((res)=>{
                    finalresult[i].push(res.data.data);
                })
            }
        }
