...
function parseSelect(ast) {
    ...
    return crudService.desc(tableName).then(tableInfo => {
        ...
        console.log(condition)
            ...
            let key;
            // if (condition.conditions[tableInfo.key]===undefined||
            //     condition.conditions[tableInfo.key][ConditionOp.EQ]===undefined||
            //     condition.conditions[tableInfo.key]===null||
            //     condition.conditions[tableInfo.key][ConditionOp.EQ]===null
            // ){
            //     key='0'
            // }else{
            //     key = condition.conditions[tableInfo.key][ConditionOp.EQ];
            // }
            key = condition.conditions[tableInfo.key][ConditionOp.EQ];
            let limit = ast.limit;
            if (limit) {
